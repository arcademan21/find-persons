        ...
        <Document>
            ...
            <Page size="A4" style={styles.page}>
                    ...
                    dataPerson.response.related_questions.map( ( question, index ) => {
                        return (
                            <View style={styles.section} key={index}>
                                ...
                                <Link src={question.displayed_url}>{question.displayed_url}</Link>
                            </View>
                        )
                    })
                }
                ...
                <View style={styles.section}>
                    ...
                    <Text style={styles.text}>
                        {language.device} : { dataPerson.response.search_parameters.device }
                    </Text>
                    <Text style={styles.text}>
                        {language.auto_location} : { dataPerson.response.search_parameters.auto_location }
                    </Text>
                    <Text style={styles.text}>
                        {language.google_domain} : { dataPerson.response.search_parameters.google_domain }
                    </Text>
                    <Text style={styles.text}>
                        {language.gl} : { dataPerson.response.search_parameters.gl }
                    </Text>
                    <Text style={styles.text}>
                        {language.hl} : { dataPerson.response.search_parameters.hl }
                    </Text>
                    <Text style={styles.text}>
                    ...
                    <Text style={styles.text}>
                        {language.action} : { dataPerson.response.search_parameters.action }
                    </Text>
                    <Text style={styles.text}>
                        {language.access_key} : { dataPerson.response.search_parameters.access_key }
                    </Text>
                    <Text style={styles.text}>
                        {language.error} : { dataPerson.response.search_parameters.error }
                    </Text>
                </View>
