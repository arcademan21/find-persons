'use client'
import Head from 'next/head';

const Terms = () => {

    return (<>

        <Head>
            <meta name="robots" content="noindex" />
        </Head>

        <div className="container py-5">
            
            <div className="page-banner my-5">
                <div className="row justify-content-center align-items-center h-100">
                    <div className="col-md-6">
                    <h1 className="text-center">Terminos y condiciones</h1>
                    </div>
                </div>
            </div>

            <main>
                <div className="page-section">
                <div className="container">
                    <div className="row align-items-center">
                    <div className="col-lg-12 py-3">
                    
                        <h2 className="title-section">Términos y condiciones del servicio y el sitio web:</h2>
                        <div className="divider"></div>
                        
                        <p>
                        Los siguientes términos y condiciones legales están disponibles en el pie de página de la
                        portada en www.find-persons.com (web o página web) y establecen tanto las condiciones de uso y
                        acceso a los servicios ofrecidos a los usuarios por la página web con respecto a la marca, como
                        los productos y los servicios propiedad de MONTAUP PUBLICISTES SLU con nombre comercial
                        ADS DIGITAL, NRT L-714323-H inscrita en el registro mercantil de Andorra con núm. de registro
                        societario 19221.
                        
                        Los servicios disponibles en nuestro portal (o la web www.find-persons.com) son proporcionados por
                        la web, una marca que es propiedad y por tanto es gestionada por completo por ADS DIGITAL
                        (la empresa), desde su sede en Carrer Esteve Albert, R. la Roureda, bloc A, 1-2a AD500 Santa
                        Coloma, Andorra La Vella, PRINCIPAT D’ANDORRA.
                        </p>
                    
                        <br />
                        
                        Al utilizar nuestro servicio, usted declara y garantiza que es el titular de los datos personales que está buscando o que tiene el consentimiento explícito del titular de dichos datos para buscar y obtener esta información. Usted entiende que la información que proporcionamos se obtiene de fuentes de acceso público y está destinada únicamente para el uso consentido y legal. Al hacer clic en &apos;Acepto&apos;, usted acepta que ha leído y entendido esta declaración y que se compromete a cumplir con todas las leyes y regulaciones de privacidad y protección de datos aplicables, incluyendo el Reglamento General de Protección de Datos (RGPD).
                        
                        <br />
                        
                        Usted acepta asumir la responsabilidad total y eximir a find-persons.com y a sus propietarios, empleados y afiliados de cualquier responsabilidad, reclamo, demanda, litigio o acción legal que pueda surgir debido al incumplimiento de las leyes y regulaciones de privacidad y protección de datos aplicables, incluyendo el RGPD, en relación con el uso de los datos obtenidos a través de nuestro sitio web.
                        
                        
                        Es necesario que el usuario lea atentamente este documento y que tenga conocimiento de que
                        la web y los términos y condiciones legales descritos en ella pueden estar sujetos a cambios y/o
                        actualizaciones, sin previo aviso.
                        
                        El uso de esta página web implica la aceptación de estos términos y condiciones del servicio en
                        cuestión, y de sus condiciones legales y políticas de privacidad, esta web está destinada a
                        personas con la edad legal necesaria, implicando que el usuario tiene la plena capacidad de
                        actuar en su propio nombre y ateniéndose a las condiciones del servicio o web.
                        
                        A continuación les especificamos las definiciones pertinentes de los elementos y expresiones
                        usados en estos términos y en el contenido de la web y sus correspondientes aplicaciones y
                        derechos:
                        <div className="divider"></div>
                        <h2 className="title-section">1.Conceptos y definiciones del sitio web:</h2>
                        En este documento,
                        "CGV": Se refiere a estas condiciones generales de venta.
                        "Empresa": significa el editor y propietario del sitio web.
                        "Sitio web": significa el servicio electrónico en línea publicado por la Compañía, accesible en la
                        dirección www.find-persons.com, y todos los elementos asociados al mismo.
                        "Servicios": significa todos los servicios, productos, contenido, funcionalidad, tecnología y
                        "Miembro": significa cualquier persona física o jurídica que tenga una cuenta en el Sitio.
                        "Espacio personal": es un espacio personal proporcionado a los miembros donde los miembros
                        pueden obtener información personal y contactar con www.find-persons.com.
                        
                        "Soporte en línea": significa todos los servicios de soporte disponibles para los Miembros, por
                        ejemplo, operadores cuya tarea es ayudar a los Miembros con cualquier solicitud realizada a
                        través de centrales telefónicas, correo electrónico o a través de las páginas de contacto de
                        nuestro sitio web.
                        
                        "Suscripción": significa una suscripción paga mensual a la que un Miembro puede suscribirse en
                        el Sitio. El socio se suscribe al servicio de forma indefinida a partir de la fecha de confirmación
                        por parte de la empresa. No hay ningún compromiso con esta suscripción y el agente puede
                        cancelar en cualquier momento.
                        
                        "Período de prueba": significa el período de veinticuatro (24) horas durante el cual el Miembro
                        puede tener acceso ilimitado a los servicios del Sitio sin pagar una tarifa de suscripción. Dichos
                        períodos de prueba siempre se incluyen en las ofertas promocionales.
                        
                        "Oferta Promocional": significa acceso ilimitado a los Servicios ofrecidos en el Sitio durante el
                        Período de Prueba. El registro de afiliados para ofertas promocionales requiere
                        automáticamente una suscripción. Las ofertas promocionales permiten a los miembros probar el
                        servicio antes de pagar la suscripción.
                        <div className="divider"></div>
                        <h2 className="title-section">2.Apertura de cuenta y consentimiento de socios:</h2>
                        <div className="divider"></div>
                        <h2 className="title-section">3.1 Condiciones</h2>
                        Debe tener al menos 18 años para acceder a nuestros Servicios y, si corresponde, debe
                        demostrarlo al abrir una cuenta.
                        
                        En lo que a usted concierne, debe tener la autoridad para obligar a la empresa que representa.
                        Usted certifica que tiene la autoridad para vincular a la empresa o entidad en cuyo nombre
                        accede a nuestros servicios y que acepta plenamente los T&amp;C
                        <div className="divider"></div>
                        <h2 className="title-section">3. Cuentas de usuario:</h2>
                        La empresa proporciona servicios a los miembros en el sitio web de conformidad con el artículo.
                        
                        Para suscribirse al servicio, los socios deben seguir los pasos del procedimiento indicado en
                        el sitio web. Deberá rellenar todos los campos que le permitan identificarse, seleccionar
                        servicios y formas de pago. Antes de utilizar este sitio web, el usuario debe asegurarse de que
                        dispone de los medios técnicos e informáticos necesarios para utilizar este sitio web y que su
                        navegador le permite acceder de forma segura a este sitio web. Asimismo, el agente deberá
                        asegurarse de que la configuración informática de su material/equipo se encuentra en buen
                        estado y no contiene virus.
                        
                        Al utilizar este sitio web y los servicios, los usuarios reconocen y confirman que han leído,
                        comprendido y aceptado las presentes condiciones generales de contratación en su totalidad y
                        sin reservas.
                        
                        Será imposible el acceso a nuestros Servicios para cualquier persona que no sea un Miembro.
                        
                        <div className="divider"></div>
                        <h2 className="title-section">
                        4.Descripción de los servicios:
                        </h2>
                        
                        El sitio es una plataforma en línea basada en la suscripción en la que, como miembro, puede
                        buscar datos sobre personas como niños, padres, profesores y otros profesionales:
                        
                        Puedes buscar datos de personas como direcciones, nombre, dirección de correo electrónico,
                        número de teléfono, cuenta de redes sociales, nombre de usuario, etc.
                        
                        Su membresía le da derecho a búsquedas ilimitadas.
                        
                        Puedes realizar tantas búsquedas como quieras al mes.
                        
                        Una búsqueda exitosa = una coincidencia con nuestra API que se mostrará como un resultado
                        detallado de los datos buscados.
                        
                        Puede obtener un informe detallado de cada búsqueda realizada con éxito.
                        
                        Obtendrá asistencia en línea que incluye todos los servicios relacionados con la gestión de la
                        cuenta, las suscripciones y las conversiones de documentos. Este servicio incluye la posibilidad
                        de contactar con asesores por correo electrónico para obtener información sobre el Sitio o los
                        Servicios.
                        
                        Todos los servicios anteriores sólo están disponibles para los miembros del sitio, lo que significa
                        suscribirse a nuestro paquete de suscripción.
                        <div className="divider"></div>
                        <h2 className="title-section">5.Precios y planes:</h2>
                        El Miembro debe pagar el importe de la Suscripción mensual o sólo el del Período de Prueba, tal
                        y como se describe en el Sitio, para poder acceder a los Servicios ofrecidos por el Sitio. Los
                        precios de la Suscripción y del Período de Prueba se indican en el Sitio con todos los impuestos
                        incluidos. Incluyen el importe del IVA aplicable el día de la suscripción.
                        
                        La Suscripción al Servicio se concluye por un período indefinido a partir de la fecha de
                        suscripción, la Suscripción es sin compromiso y puede ser cancelada en cualquier momento por
                        el Miembro.
                        
                        El importe de la Suscripción así como el del Periodo de Prueba pueden ser modificados en
                        cualquier momento por el Sitio. El Socio será informado con antelación 10 días antes de que
                        finalice su Suscripción actual, por lo que será libre de renovar o no su Suscripción.
                        
                        El Periodo de Prueba así como la Suscripción son pagaderos por adelantado mediante
                        domiciliación bancaria en la forma de pago indicada en el Sitio. Si el sitio web constata un
                        incumplimiento de pago por parte del afiliado, volveremos a pagar. En caso de un nuevo
                        incumplimiento de pago, el Sitio se reserva el derecho de poner fin a los Servicios de los que se
                        beneficia el Miembro, sin indemnización, pero sin perjuicio de su derecho a reclamar el pago de
                        las sumas adeudadas por el Miembro.
                        
                        El Sitio utiliza un proveedor de servicios especializado en la seguridad de los pagos en línea
                        para garantizar la seguridad de los pagos realizados en el Sitio. La transacción con tarjeta de
                        crédito realizada entre el diputado y el sistema de pago seguro está codificada, la empresa
                        nunca tendrá acceso al número completo de la tarjeta de crédito del diputado. Esto garantiza al
                        diputado la total confidencialidad de su información bancaria.
                        
                        El Socio garantiza a la Empresa que dispone de las autorizaciones necesarias para utilizar el
                        medio de pago al realizar el Pedido.
                        <div className="divider"></div>
                        <h2 className="title-section">6.Suscripción a los servicios:</h2>
                        Todos los Servicios ofrecidos por el Sitio se describen con la mayor exactitud posible más arriba.
                        Nos reservamos el derecho de cambiar el contenido del Sitio en cualquier momento sin previo
                        aviso a los Miembros.
                        
                        El afiliado encontrará los servicios a los que tiene acceso en el resumen de su pedido.
                        
                        Antes de validar la suscripción a nuestra oferta promocional, el miembro debe confirmar su
                        aceptación de estas T&amp;C y las CGV marcando la casilla de validación prevista a tal efecto. Junto
                        al icono de validación del pedido aparece una nota de pago clara y legible para garantizar que el
                        afiliado reconoce explícitamente su obligación de pagar el pedido.
                        
                        Tras la aceptación de las CGV por parte del Usuario y la validación de su suscripción a nuestra
                        oferta promocional con obligación de pago, se celebrará el contrato entre la Empresa y el
                        Usuario que les vinculará mutuamente.
                        
                        La Empresa enviará al Socio una confirmación de pedido por correo electrónico en la que se
                        resumen todos los detalles de su pedido, así como sus identificadores que permiten acceder a
                        su espacio personal.
                        <div className="divider"></div>
                        <h2 className="title-section">7.Acceso a los servicios y a la cuenta de usuario:</h2>
                        La Empresa se compromete a proporcionar al Socio el acceso a los Servicios según lo acordado
                        en estas CGU. Servicios.
                        
                        Tras la suscripción del Miembro al periodo de prueba, se le proporcionarán los identificadores
                        necesarios para que pueda acceder a su Espacio Personal. El diputado es totalmente
                        responsable de la confidencialidad de sus identificadores para preservar la seguridad de su
                        espacio personal. El diputado está obligado a informar inmediatamente a la empresa de
                        cualquier uso no autorizado de sus identificadores. El diputado será el único responsable de
                        cualquier pérdida o daño que se derive de no haber mantenido en secreto sus datos de acceso.
                        
                        La Empresa no será responsable si la inaccesibilidad de los Servicios se debe a un tercero
                        ajeno a su voluntad.
                        <div className="divider"></div>
                        <h2 className="title-section">8.Atención al cliente:</h2>
                        Tras la suscripción de un periodo de prueba o de un abono, el usuario tendrá la posibilidad de
                        ponerse en contacto con el servicio de asistencia en línea para obtener información o presentar
                        una reclamación.
                        <div className="divider"></div>
                        <h2 className="title-section">9.Contraseña:</h2>
                        Usted es responsable de la contraseña de su cuenta y la mantiene confidencial. Además, acepta
                        notificarnos cualquier acceso no autorizado a su cuenta o cualquier violación de la seguridad de
                        su cuenta. Esto incluye la pérdida o el robo de su información de acceso.
                        <div className="divider"></div>
                        <h2 className="title-section">10.Obligaciones de los miembros</h2>
                        El Socio se compromete a respetar los términos de estas CGV.
                        El Usuario se compromete a utilizar el Sitio y los Servicios de acuerdo con las instrucciones de
                        la Empresa.
                        
                        En beneficio de todos los diputados, el uso del sitio está sujeto a diversas normas. Por lo tanto,
                        al utilizar este sitio, usted acepta :
                        
                        Utilizar el Sitio de forma lícita y, en general, no hacer nada que pueda ser contrario a la
                        Legislación Aplicable y a estas Condiciones Generales.
                        
                        No infringir los derechos del Sitio y de sus Afiliados, incluidos sus derechos de propiedad
                        intelectual.
                        
                        No intentar acceder sin autorización al sistema informático del Sitio, no utilizar ningún
                        dispositivo, software u otro elemento que pueda afectar al buen funcionamiento del Sitio.
                        
                        No denigrar el Sitio y/o los Servicios así como a la Empresa en las redes sociales y cualquier
                        otro medio de comunicación.
                        
                        No abrir una cuenta a nombre de otra persona que no sea usted.
                        
                        La Empresa se reserva el derecho de retirar el acceso de un Miembro al Sitio y de tomar todas
                        las medidas legales y penales contra él si, por cualquier motivo, la Empresa considera que el
                        Miembro no respeta las presentes CGV.
                        <div className="divider"></div>
                        <h2 className="title-section">11.Derecho de desistimiento y abonos.</h2>
                        El Socio reconoce que la ejecución de los Servicios prestados por la Empresa tendrá lugar antes
                        de que finalice el plazo legal de desistimiento y, por tanto, renuncia expresamente a su derecho
                        legal de desistimiento.
                        
                        En el caso de una Suscripción con una Oferta promocional y a partir de la validación del Pedido,
                        el Socio dispone de un plazo de cuarenta y ocho horas para ponerse en contacto con la
                        Empresa y solicitar la cancelación de cualquier Suscripción mensual.
                        
                        Si un Socio no está satisfecho con los servicios ofrecidos por la Empresa, puede utilizar la
                        opción "Satisfecho o Reembolsado" y solicitar el reembolso de una o varias mensualidades de
                        su Suscripción poniéndose en contacto con la Empresa por teléfono, chat o correo electrónico.
                        
                        Tras la finalización de la Suscripción mensual antes de que termine el periodo de prueba de
                        cuarenta y ocho horas o en caso de ejercer la opción "Satisfecho o le devolvemos su dinero", la
                        Empresa reembolsará al Socio a través del mismo medio de pago que utilizó para el Pedido
                        inicial. Si el reembolso del Socio es imposible debido a un cambio en los datos bancarios del
                        Socio desde la fecha del Pedido inicial, la Compañía no será responsable. En caso de que se
                        produzca esta situación, se invita al diputado a ponerse en contacto con la empresa para
                        acordar otra solución de reembolso. Si el Socio es el origen de un fraude bancario consistente
                        en una declaración falsa de pérdida o robo de su tarjeta bancaria tras la compra de un Servicio
                        en el Sitio, la Empresa no estará obligada a reembolsar al Socio. Además, el Socio seguirá
                        siendo responsable del pago de todas las sumas debidas a la Sociedad.
                        <div className="divider"></div>
                        <h2 className="title-section">12.Cancelación de suscripción:</h2>
                        La Suscripción contratada por el Afiliado es una suscripción que no incluye un compromiso de
                        tiempo, por lo que el Afiliado sólo tiene que notificar a la Compañía por correo electrónico, chat o
                        teléfono su deseo de terminar la Suscripción para que ésta termine. El Socio también puede dar
                        por terminada su Suscripción en el Sitio.
                        
                        Cualquier rescisión surtirá efecto al final del mes de la suscripción en curso, que sigue siendo
                        exigible. A partir de la fecha efectiva de rescisión, el usuario dejará de tener acceso a la zona de
                        usuarios y a los servicios.
                        <div className="divider"></div>
                        <h2 className="title-section">13.Responsabilidad</h2>
                        La empresa pondrá en marcha todas las medidas necesarias para la correcta ejecución del
                        servicio. No obstante, la Sociedad no podrá ser considerada responsable de la mala ejecución o
                        de la no ejecución de la totalidad o de una parte del Servicio si ello se debe al Socio, al hecho
                        imprevisible e insuperable de un tercero ajeno al Pedido o a un caso de fuerza mayor. En caso
                        de que la Sociedad sea considerada responsable, no aceptará indemnizar al Socio por los
                        daños directos o indirectos sin prueba.
                        
                        La Compañía hará todo lo posible para permitir la accesibilidad al Sitio las 24 horas del día, los 7
                        días de la semana. En ningún caso se podrá responsabilizar a la Empresa de la indisponibilidad
                        debida a limitaciones relacionadas con el funcionamiento de la red de Internet, a operaciones de
                        mantenimiento, a cualquier avería o error, a cualquier acontecimiento ajeno a su voluntad o a
                        cualquier caso de fuerza mayor que escape al control razonable de la Empresa.
                        
                        En el Sitio pueden aparecer enlaces a otros sitios no publicados y no controlados por la
                        Empresa. En ningún caso la empresa puede ser considerada responsable del funcionamiento, el
                        contenido o cualquier elemento presente u obtenido a través de estos sitios. La presencia de
                        estos enlaces en el Sitio no debe interpretarse como una aprobación expresa o implícita por
                        parte de la Empresa de los contenidos o servicios prestados por estos sitios. La empresa no es
                        responsable de la disponibilidad de dichos sitios ni del contenido, la publicidad, los productos u
                        otros materiales de dichos sitios.
                        
                        Si la dirección de correo electrónico del Socio rechaza, por ejemplo, debido al antispam, los
                        mensajes de correo electrónico enviados por la Empresa, y en particular la copia del recibo de
                        pago o el resumen del Pedido o, más generalmente, cualquier otro mensaje de correo
                        electrónico de la Empresa, ésta no será responsable en modo alguno.
                        <div className="divider"></div>
                        <h2 className="title-section">14.Datos personales:</h2>
                        Para que el proceso de pedido se desarrolle sin problemas, es imprescindible que el Sitio tenga
                        acceso a la siguiente información: apellido, nombre, dirección electrónica (e-mail).
                        
                        Los proveedores de servicios, y en particular los anfitriones establecidos fuera de la Unión
                        Europea, pueden tener acceso a los datos personales de los diputados. La empresa ha
                        celebrado acuerdos con estos proveedores de servicios para la transferencia de datos
                        personales, que incluyen cláusulas contractuales estándar aprobadas por la Comisión Europea.
                        
                        Los socios pueden ejercer su derecho de oposición, rectificación, supresión y acceso a sus
                        datos personales, así como su derecho de oposición por motivos legítimos, en las condiciones
                        previstas por la ley, enviando un correo electrónico a la Sociedad a la dirección
                        
                        suppor@find-persons.com, especificando sus apellidos, nombre, dirección de correo
                        electrónico y número de pedido.
                        
                        La información relativa al Socio sólo será transmitida por la Sociedad a los proveedores de
                        servicios para el mantenimiento, el alojamiento del Sitio y la ejecución de los pedidos dentro del
                        límite de la información estrictamente necesaria para la correcta ejecución de estos últimos.
                        
                        De acuerdo con la deliberación de la CNIL nº 2013-378 del 5 de diciembre de 2013, la Sociedad
                        también informa al Socio que las cookies registran cierta información que se almacena en la
                        memoria del disco duro del Socio. Esta información se utiliza para llevar a cabo estadísticas
                        sobre la audiencia del sitio y para ofrecer Servicios de acuerdo con los ya seleccionados por el
                        Miembro durante las visitas anteriores. Un mensaje de advertencia, en forma de banner,
                        pregunta a cada persona que visita el Sitio, de antemano, si el Miembro desea aceptar las
                        cookies. Estas cookies no contienen ninguna información confidencial sobre los Miembros del
                        Sitio.
                        
                        Cualquiera que sea la página del Sitio a la que se dirija el diputado directamente desde un motor
                        de búsqueda, será informado:
                        
                        la finalidad precisa de las cookies utilizadas.
                        
                        de la posibilidad de oponerse a estas cookies y de modificar sus parámetros haciendo clic en un
                        enlace presente en el banner.
                        
                        y del hecho de que la continuación de su navegación implica su acuerdo con el depósito de
                        cookies en su terminal.
                        
                        Sin el consentimiento previo del diputado, las cookies no serán depositadas ni leídas
                        
                        si el Usuario va al Sitio (página de inicio o directamente a otra página del Sitio desde un motor
                        de búsqueda, por ejemplo) y no continúa su navegación: una simple ausencia de acción no
                        podría considerarse como una manifestación de voluntad.
                        
                        o si hace clic en el enlace del banner que le permite configurar las cookies y, en su caso,
                        rechaza el depósito de cookies.
                        <div className="divider"></div>
                        <h2 className="title-section">15.Propiedad intelectual:</h2>
                        Este Sitio es operado por la Compañía y sus afiliados.
                        
                        Elementos como logotipos, marcas, nombres comerciales, imágenes, textos, ilustraciones,
                        archivos de audio, archivos de vídeo, así como la selección, coordinación y combinación de
                        estos elementos son propiedad exclusiva de la Compañía y están protegidos por derechos de
                        autor, derecho de marcas, diseños y/o cualquier otro derecho de propiedad intelectual. Todos
                        estos derechos están reservados para todo el mundo.
                        
                        Una vez que es miembro, se convierte en beneficiario de una licencia no exclusiva, revocable,
                        personal e intransferible para utilizar el Sitio, para su uso personal, o el de la empresa que
                        representa, y de acuerdo con la finalidad del Sitio.
                        
                        Cualquier pregunta o comentario relacionado con un sitio distinto al de la Compañía debe
                        dirigirse a los operadores de estos sitios, quedando prohibido cualquier enlace a este Sitio sin la
                        autorización previa y expresa de la Compañía.
                        
                        Queda terminantemente prohibida la utilización por parte del Socio de las denominaciones
                        sociales, marcas y signos distintivos pertenecientes a la Sociedad, sin el previo y expreso
                        consentimiento de la misma.
                        <div className="divider"></div>
                        <h2 className="title-section">16.Ley y jurisdicciones aplicables:</h2>
                        Estos términos y condiciones se regirán e interpretarán de acuerdo con la legislación español,
                        sin tener en cuenta los principios de conflicto de leyes.
                        
                        En el caso de cualquier disputa que surja de o en conexión con la interpretación y/o ejecución de
                        estos Términos y Condiciones, el Miembro puede elegir someter la disputa con la Compañía a
                        un procedimiento de mediación convencional o cualquier otro procedimiento alternativo.
                        
                        Método de resolución de conflictos. El diputado puede acudir a los Tribunales Andorranos, (Principat d&apos; Andorra)
                        El Afiliado reconoce que antes de emprender cualquier procedimiento de mediación, debe
                        ponerse en contacto con la Compañía por teléfono, chat o correo electrónico en la siguiente
                        dirección: suppor@find-persons.com para hacer valer su derecho al Reembolso (véase 10.
                        Ejercicio de la opción "Satisfecho o Reembolsado").
                        
                        Si este procedimiento de mediación fracasa o si el diputado desea llevar el asunto a los
                        tribunales, se aplicarán las normas del Código de Procedimiento Civil.
                        
                    



            
                        
                    </div>
                    
                    </div>
                </div> 
                </div> 
            </main>


        </div>
    </>)

}

export default Terms